...
/* Array that holds data registers*/
STATIC volatile uint32* DataRegArray[6] = {
	&GPIO_PORTA_DATA_REG,
	&GPIO_PORTB_DATA_REG,
	&GPIO_PORTC_DATA_REG,
	&GPIO_PORTD_DATA_REG,
...
void Dio_WriteChannel(Dio_ChannelType ChannelId, Dio_LevelType Level)
{
	...
	if(FALSE == error)
	{
		...
		Port_ptr = DataRegArray[Dio_PortChannels[ChannelId].Port_Num];
...
Dio_PortLevelType Dio_ReadPort(Dio_PortType PortId)
{
    ...
    if(error == FALSE){
        Port_ptr = DataRegArray[Dio_Ports[PortId]];
    }
	else{
...
Dio_PortLevelType Dio_ReadChannelGroup ( const Dio_ChannelGroupType* ChannelGroupIdPtr ){
    ...
    if(error == FALSE){
        Port_ptr = DataRegArray[ChannelGroupIdPtr->PortIndex];
    }
	else{
